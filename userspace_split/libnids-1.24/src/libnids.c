...
static void nids_syslog(int type, int errnum, struct ip *iph, void *data)
{
	...
	if (errnum != NIDS_WARN_TCP_HDR)
	    ...
	    syslog(nids_params.syslog_level, "%s,from %s to %s\n",
		   nids_warnings[errnum], saddr, daddr);
	#ifdef OSPLIT
	if(errnum ==NIDS_WARN_TCP_TOOMUCH)
	printf("resize the max_stream please!\n");
	#endif
	break;
...
#ifdef OSPLIT
	 if(store_flag==0) {
		 // printf("NO.%ld pcap packet is dropped by Osplit!\n",cur_seq);
		 tot_filter_nr++;
	 }
	 else
		 store_flag=0;
...
if (!nids_params.ip_filter(iph, len))
	return;
#ifdef OSPLIT
#ifdef CHECK_IPHDR_DISABLED
	 /* may filter a lot of  packets when tcpdumping  trace without "-s 0" */
...
#ifdef OSPLIT
#ifdef NO_ANY_CHECK
static void split_trace(u_char * data, int len) {
	
	u_char res;
	u_int src,dest;
	u_char xor[8];
	struct ip *iph=(struct ip*)data;
	src=iph->ip_src.s_addr;
	...
	*(u_int*)(xor+4)=dest;
	...
	for(i=1;i<8;i++)
		res=*(xor+i)^res;
	res=res%SPLIT_FILE_NUM;
	/*store to trace splited*/
  	FILE *fp=split_file[res];
	/*no fragment analysis*/
	write_pcap_hdr(fp,(char*)nids_last_pcap_header,sizeof(struct pcap_sf_pkthdr));
	write_ip(fp,(char*)iph,ntohs(iph->ip_len),(char*)nids_last_pcap_header);
	store_flag=1;
}
#endif
#endif
...
static void process_udp(char *data)
{
    ...
    int ulen;
	 ...
	 FILE *fp=split_file[(UTCP_file_idx++)%SPLIT_FILE_NUM];
	 /*write all fragment(s) to fp trace file*/
	 if(is_frag==0) {
	 	 
		 write_pcap_hdr(fp,(char*)nids_last_pcap_header,sizeof(pcap_sf_pkthdr_t));
		 write_ip(fp,(char*)iph,ntohs(iph->ip_len),(char*)nids_last_pcap_header);
	 }
	 else {
		 /*fragments*/
		 while(frag_tag!=NULL) {
			 write_pcap_hdr(fp,(char*)(&(frag_tag->pcap_header)),sizeof(pcap_sf_pkthdr_t));
			 write_ip(fp,frag_tag->skb->data,frag_tag->wtrace_len,(char*)(&(frag_tag->pcap_header)));
			 free(frag_tag);
...
static void gen_ip_proc(u_char * data, int skblen)
{
 ...
 #ifdef OSPLIT
	{
		...
		if(is_frag==0) {
			write_pcap_hdr(fp,(char*)nids_last_pcap_header,sizeof(pcap_sf_pkthdr_t));
			write_ip(fp,(char*)data,ntohs(((struct ip *)data)->ip_len),(char*)nids_last_pcap_header);
		}
		else {
			/*fragments*/
			while(frag_tag!=NULL) {
				write_pcap_hdr(fp,(char*)(&(frag_tag->pcap_header)),sizeof(pcap_sf_pkthdr_t));
				write_ip(fp,(char*)frag_tag->skb->data,frag_tag->wtrace_len,(char*)(&(frag_tag->pcap_header)));
				free(frag_tag);
				frag_tag=ip_frag_next;
				if(ip_frag_next!=NULL)
					ip_frag_next=ip_frag_next->next;
...
static void init_procs()
{
	...
	#ifdef OSPLIT
	#ifdef NO_ANY_CHECK
    ip_frag_procs->item = split_trace;
	#else
