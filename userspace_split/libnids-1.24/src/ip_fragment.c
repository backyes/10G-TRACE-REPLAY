...
ip_defrag(struct ip *iph, struct sk_buff *skb)
{
 ...
 #ifdef OSPLIT
	tfp->pcap_header=*nids_last_pcap_header;
 #endif
  /*
...
#ifdef OSPLIT
	  /*
		* set the fragments RAW packet for L4 processing after HASH
		*/
	  is_frag=1;
	  struct ipfrag *fp;
	  struct ipfrag *dst_ipfrag;
	  fp=qp->fragments;
	  while(fp!=NULL) {
		  dst_ipfrag=(struct ipfrag*)malloc(sizeof(struct ipfrag)+sizeof(struct sk_buff)+fp->len+qp->ihlen);
		  dst_ipfrag->wtrace_len=fp->len+qp->ihlen;
		  dst_ipfrag->skb=(struct sk_buff *)(dst_ipfrag+1);
		  dst_ipfrag->skb->data=(char*)(dst_ipfrag->skb+1);
		  dst_ipfrag->pcap_header=fp->pcap_header;
		  memcpy(dst_ipfrag->skb->data,fp->skb->data,fp->len+qp->ihlen);
		  /*add to this_fragments, which one has the sending priority??, from head to tail*/
		  if(this_fragments==NULL) {
			  dst_ipfrag->next=NULL;
			  this_fragments=dst_ipfrag;
			  this_frag_tail=dst_ipfrag;
		  } 
		  else {
			  this_frag_tail->next=dst_ipfrag;
			  dst_ipfrag->next=NULL;
			  this_frag_tail=dst_ipfrag;
		  }
		  fp=fp->next;
